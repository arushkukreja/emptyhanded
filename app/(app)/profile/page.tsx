...
export default async function ProfilePage() {
  ...
  const { data: profile } = await supabase.from("users").select("display_name, avatar_path, email_reminders_enabled").eq("id", user.id).maybeSingle();
  ...
  return (
    <div className="min-h-screen bg-[#fbfaf7] pb-24">
      <div className="mx-auto max-w-4xl px-4 py-8 min-[390px]:px-5 sm:px-8 sm:py-10">
        ...
        <ReminderPreferences enabled={profile?.email_reminders_enabled !== false} />
      </div>
    </div>
  );
}
