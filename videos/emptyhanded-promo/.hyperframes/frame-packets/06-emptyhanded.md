...
- scene: Product surfaces and orange accents clear into the emptyhanded. wordmark and final promise.
- voiceover: "Never show up empty handed again."
- duration: 5s
- transition_in: crossfade
- status: outline
- src: compositions/frames/06-emptyhanded.html
- type: cta
- persuasion: Memorable brand promise with a single next step
- beat: warmth + motivation
- blueprint: compose
- asset_candidates:
- focal: emptyhanded. wordmark
- roles: wordmark = foreground hero · orange dot = accent subject · promise = supporting
- sfx: impact-bass-1

Compose: assemble only the lowercase wordmark and its oversized visible orange terminal dot, then spend the final half on a still lockup.
Scene 1 (0.0–1.3s): Deep navy field; the promise `NEVER SHOW UP` lands word-by-word across the upper third through a quiet **per-word staggered reveal** (`dynamic-content-sequencing`). Thin parchment rules anchor both edges; no logo yet.
Scene 2 (1.3–2.7s): On “empty handed,” the promise collapses into the center through a restrained **scale-swap** (`scale-swap-transition`); lowercase `emptyhanded` pulls out from the same anchor with a short **waterfall entry** (`waterfall-entry`), Fraunces display filling roughly 65% of frame width.
Scene 3 (2.7–3.4s): The terminal orange dot lands larger and brighter than body punctuation using a smooth **spring-pop entrance** (`spring-pop-entrance`) without bounce; a single warm glow blooms once behind the finished lockup (`ambient-glow-bloom`) and resolves.
Scene 4 (3.4–5.0s): Final lockup holds dead static: `emptyhanded.` centered, `Never show up empty handed again.` beneath, and a restrained `GET STARTED FREE` CTA aligned on the lower safe rail. No fade to black.

narrativeRole: Resolve the story into the brand's one-line promise and hold long enough to remember it.
keyMessage: EmptyHanded makes thoughtful giving feel automatic.
