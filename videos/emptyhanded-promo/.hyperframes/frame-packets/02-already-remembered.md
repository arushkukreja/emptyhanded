# Frame packet: 02-already-remembered
...
## Frame 2 — Already remembered

- scene: The close-up birthday detail pulls back to reveal Alex's signed-in EmptyHanded dashboard.
- voiceover: "And this time, Alex doesn't have to remember alone."
- duration: 5s
- transition_in: zoom-through
- status: outline
- src: compositions/frames/02-already-remembered.html
- type: product_intro
- persuasion: Friction reduction through a visible remembered occasion
- beat: relief + recognition
- blueprint: compose
- asset_candidates: assets/scroll-000.png — dashboard opening with Alex Morgan, Maya, and the Up next panel
- focal: assets/scroll-000.png
- roles: scroll-000 = full product surface and close-up detail
- sfx: whoosh-short

Compose: begin inside Maya's Up next card and reveal Alex's complete signed-in dashboard with one decelerating zoom-out and a locked-wide payoff; never zoom back in.
Scene 1 (0.0–1.6s): Extreme close-up on Maya's seven-day card, cropped full-bleed with no surrounding chrome; `REMEMBERED` appears at the left edge only when the VO says “remember,” using **per-word staggered reveal** (`dynamic-content-sequencing`). The close-up sharpens through **depth-of-field / selective-blur** (`depth-of-field-blur`).
Scene 2 (1.6–3.2s): On “doesn't have to remember alone,” one continuous **pan / focus-lock** outward reveal (`viewport-change`) decelerates from the card to the full signed-in dashboard, Alex Morgan's identity landing top-right and Maya's card remaining the visual anchor. Layered-depth composition with the dashboard filling at least 75% of frame.
Scene 3 (3.2–5.0s): Camera locks completely on the wide dashboard; an orange selection rail draws from Alex to Maya and a compact `ALREADY ON IT` label reveals beside the card via **SVG self-draw** (`svg-path-draw`) and **keyword glow** (`asr-keyword-glow`). Hold the resolved product proof still.

narrativeRole: Land the promise by showing that EmptyHanded is already holding the occasion for Alex.
keyMessage: EmptyHanded remembers before the deadline becomes a problem.
