...
export async function generateRecommendations(profile: ProfileInput): Promise<Recommendation[]> {
  ...
  const catalogFields = "asin, name, budget_tier, archetype_tags, age_group";
  const noCatalogFilters = profile.archetypes.length === 0 && !profile.age_range;
  const [archetypeResult, ageResult, fallbackResult] = await Promise.all([
    profile.archetypes.length > 0
      ? admin.from("products").select(catalogFields).overlaps("archetype_tags", profile.archetypes).limit(10)
      : Promise.resolve({ data: [] as CatalogCandidate[] }),
    profile.age_range
      ? admin.from("products").select(catalogFields).eq("age_group", profile.age_range).limit(10)
      : Promise.resolve({ data: [] as CatalogCandidate[] }),
    noCatalogFilters
      ? admin.from("products").select(catalogFields).limit(10)
      : Promise.resolve({ data: [] as CatalogCandidate[] })
  ]);
  ...
  const userPrompt = [
    ...
    profile.budget_tier ? `Budget tier: ${profile.budget_tier}` : null,
    profile.past_gifts ? `Past gifts given: ${profile.past_gifts}` : null,
    "",
    candidates.length
      ? `Catalog candidates you may pick from or suggest alternatives to:\n${candidates
      ...
      responseJsonSchema: {
        ...
        items: {
          ...
          properties: {
            ...
            asin: { anyOf: [{ type: "string" }, { type: "null" }] },
  ...
  const knownAsins = new Set(candidates.map(candidate => candidate.asin));
  return recommendations.map(recommendation => ({
    ...recommendation,
    asin: recommendation.asin && knownAsins.has(recommendation.asin) ? recommendation.asin : null
  }));
}
