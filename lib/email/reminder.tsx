...
export function renderReminderEmail(p: ReminderProps): { subject: string; html: string } {
  ...
  const recsHtml = p.recommendations.map((recommendation, index) => `
    <tr><td style="padding:0 0 12px;">
      <table class="pick-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width:100%;border:1px solid #EFEDE8;border-radius:14px;background:#FAFAF9;overflow:hidden;">
        <tr>
          <td class="pick-image-cell" width="84" style="width:84px;padding:12px 0 12px 12px;">
            <div class="pick-image-box" style="width:72px;height:72px;border-radius:12px;background:#F5F0E8;color:#9B7654;text-align:center;font-size:25px;line-height:72px;overflow:hidden;">
          ...
          <td class="pick-copy" style="padding:14px 10px 14px 14px;">
            ...
            <div style="margin-top:4px;font-weight:700;color:#0F172A;font-size:15px;line-height:1.35;">${escapeHtml(recommendation.product_name)}</div>
            ${recommendation.reason ? `<div class="pick-reason" style="font-size:13px;color:#64748B;font-style:italic;margin-top:4px;line-height:1.5;">${escapeHtml(recommendation.reason)}</div>` : ""}
          </td>
...
</head><body style="margin:0;padding:0;background:#EEEAE3;font-family:'Plus Jakarta Sans',Arial,sans-serif;color:#0F172A;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;">
  <table class="email-shell" role="presentation" width="100%" cellpadding="0" cellspacing="0" style="width:100%;background:#EEEAE3;padding:32px 12px;">
    <tr><td align="center">
      <table class="email-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#FFFFFF;border-radius:18px;overflow:hidden;border:1px solid #E7E5E0;box-shadow:0 12px 32px rgba(15,23,42,.12);">
        <tr><td class="email-header" align="center" style="background:#0F172A;padding:22px 28px;color:#FFFFFF;">
        ...
        <tr><td class="email-body" style="padding:36px 30px 30px;">
          ...
          <p class="email-intro" style="margin:0 0 26px;font-size:14px;line-height:1.7;color:#64748B;">${escapeHtml(firstName)}'s day is coming up on ${escapeHtml(p.formatted_date)}. Here are a few thoughtful ideas chosen from what you told us.</p>
        ...
        <tr><td class="email-footer" align="center" style="padding:26px 28px;background:#F5F4F1;border-top:1px solid #EFEDE8;">
          <div style="font-family:Georgia,serif;font-size:19px;font-weight:700;letter-spacing:-.04em;color:#0F172A;">emptyhanded<span style="color:#F59E0B;">.</span></div>
