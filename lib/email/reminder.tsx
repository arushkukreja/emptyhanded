      ...
      <table class="pick-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#FAFAF9" style="width:100%;border:1px solid #EFEDE8;border-radius:14px;background-color:#FAFAF9;overflow:hidden;">
        <tr>
          <td class="pick-image-cell" width="84" style="width:84px;padding:12px 0 12px 12px;">
            <div class="pick-image-box" style="width:72px;height:72px;border-radius:12px;background-color:#F5F0E8;background-image:linear-gradient(#F5F0E8,#F5F0E8);color:#9B7654;text-align:center;font-size:25px;line-height:72px;overflow:hidden;">
              ${recommendation.image_url ? `<img class="pick-image" src="${escapeHtml(recommendation.image_url)}" width="72" height="72" alt="" style="display:block;width:72px;height:72px;object-fit:contain;padding:8px;box-sizing:border-box;" />` : p.occasion_emoji}
            </div>
          </td>
...
<html><head>
  ...
  <style>
    ...
    [data-ogsc] .email-intro, [data-ogsc] .email-footer-copy { color:#CBD5E1 !important; }
    ...
    <tr><td align="center">
      <table class="email-card" role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="max-width:560px;width:100%;background-color:#FFFFFF;border-radius:18px;overflow:hidden;border:1px solid #E7E5E0;box-shadow:0 12px 32px rgba(15,23,42,.12);">
        ...
        <tr><td class="email-body" bgcolor="#FFFFFF" style="padding:36px 30px 30px;background-color:#FFFFFF;">
          <div style="text-align:center;"><div style="display:inline-block;padding:7px 12px;border-radius:999px;background:#FEF3C7;color:#B45309;font-size:10px;font-weight:700;letter-spacing:.07em;text-align:center;">${escapeHtml(firstName.toUpperCase())}'S ${escapeHtml(p.occasion_label.toUpperCase())} · 7 DAYS</div></div>
          <h1 class="email-heading" style="margin:20px 0 10px;font-family:Georgia,serif;font-size:30px;line-height:1.12;letter-spacing:-.03em;color:#0F172A;">Three picks for ${escapeHtml(firstName)}.</h1>
          <p class="email-intro" style="margin:0 0 26px;font-size:14px;line-height:1.7;color:#475569;">Coming up on ${escapeHtml(p.formatted_date)}.</p>
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${recsHtml}</table>
          <div style="text-align:center;margin-top:20px;"><a class="email-cta" href="${escapeHtml(`${appUrl}/events/${encodeURIComponent(p.event_id)}`)}" style="color:#B45309;font-size:13px;font-weight:600;text-underline-offset:4px;">See all picks →</a></div>
        </td></tr>
